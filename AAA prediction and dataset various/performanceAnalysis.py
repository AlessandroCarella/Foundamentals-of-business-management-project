...
from predictiveModel import getRandomForestClassifier, getKnnClassifier#, getNaiveBayesClassifier, getSVMClassifier

def getDataframe() -> pd.DataFrame:
    dataset = pd.read_csv ("resampledDataset.csv")
    return dataset
...
def performanceAnalysis(numberOfCrossValidations:int):
    ...
    X = dataset.drop("totalAssets", axis=1)  # Assuming "label" is the target column
    
    # Get the target (label) data from the dataset
    y = dataset["totalAssets"]
